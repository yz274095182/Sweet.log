const { deleteGlucoseRecord, getThresholdRules, listGlucoseRecords, updateGlucoseRecord } = require("../../utils/cloud");
const { GLUCOSE_TAGS, getTagLabel } = require("../../utils/tags");
const { getStatusText, getThresholdForTag, judgeGlucoseStatus } = require("../../utils/thresholds");
...
Page({
  ...
  onSelectView(event) {
    const selectedView = event.currentTarget.dataset.key;
  ...
  onSelectStatus(event) {
    const selectedStatus = event.currentTarget.dataset.key;
...
function getTagOptions(activeKey) {
  return [
...
function getViewState(selectedView) {
  return {
    selectedView,
...
function toDisplayRecord(record, thresholds) {
...
function getTableRows(records, startDate, endDate) {
...
function getTableCell(records, tag) {
...
function formatTrendPointLabel(date) {
  const hour = `${date.getHours()}`.padStart(2, "0");
  const minute = `${date.getMinutes()}`.padStart(2, "0");
  return `${hour}:${minute}`;
}
