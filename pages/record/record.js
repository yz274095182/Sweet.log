...
const { GLUCOSE_TAGS, getTagIconPath, getTagLabel, inferTagByTime } = require("../../utils/tags");
const {
...
Page({
  ...
  async onShow() {
    ...
    const selectedTag = this.data.userSelectedTag ? this.data.selectedTag : inferTagByTime();
  ...
  async onSelectTag(event) {
    const selectedTag = event.currentTarget.dataset.key;
  ...
  async onDatetimeChange(event) {
    const datetimeValue = event.detail.value.map(Number);
    const state = buildDatetimeStateFromValue(this.data.datetimeColumns, datetimeValue);
  ...
  async loadThresholds() {
    try {
      ...
      if (!thresholds.length) {
        thresholds = profile && profile.isPregnant ? getPregnancyThresholds() : getNonPregnancyThresholds();
  ...
  refreshVisibleTags() {
    ...
    const visibleTags = GLUCOSE_TAGS.map((tag) => ({
      ...tag,
      active: tag.key === selectedTag,
      displayIconPath: getTagIconPath(tag.key, tag.key === selectedTag)
    }));

    this.setData({ visibleTags });
  }
});

function getRecommendedTags(selectedTag) {
...
function getDefaultValue(latestSameTag, recentSameTagRecords) {
  if (latestSameTag && latestSameTag.value) {
...
function getValueHint(latestSameTag, recentSameTagRecords) {
  if (latestSameTag && latestSameTag.value) return "来自上一次同标签记录，可直接修改";
  if (recentSameTagRecords.length) return "来自近 7 天平均值，可直接修改";
  return "建议值，可直接修改";
}

function formatPreviousRecordText(record) {
...
function getHeroImagePath(isPregnant) {
...
function confirmOverwriteRecord(record) {
...
function clampGlucoseValue(value) {
...
function formatThresholdText(tag, rule) {
  if (!rule.enabled || (rule.min === null && rule.max === null)) {
    return "目标：未设置警戒线";
  }

  if (rule.min !== null && rule.max !== null) {
    return `目标：${rule.min}-${rule.max} mmol/L`;
  }

  if (rule.max !== null) {
    return `目标：不高于 ${rule.max} mmol/L`;
  }

  return `目标：不低于 ${rule.min} mmol/L`;
}

function buildDatetimeState(date) {
...
function buildDatetimeStateFromValue(columns, value) {
...
function getDateFromDatetimeState(columns, value) {
  const year = parseInt(columns[0][value[0]], 10);
  const month = parseInt(columns[1][value[1]], 10);
...
function buildDatetimeColumns(year, month) {
  const now = new Date();
  const years = [];
