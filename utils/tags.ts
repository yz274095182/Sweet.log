import { GlucoseTag, GlucoseTagKey } from "../types/index";

export const GLUCOSE_TAGS: GlucoseTag[] = [
...
const LEGACY_TAG_LABELS: Partial<Record<GlucoseTagKey, string>> = {
  night: "凌晨"
};

export function getTagLabel(key: GlucoseTagKey): string {
  return GLUCOSE_TAGS.find((tag) => tag.key === key)?.label || LEGACY_TAG_LABELS[key] || key;
}

export function getTagIconPath(key: GlucoseTagKey, active = false): string {
...
export function inferTagByTime(date = new Date()): GlucoseTagKey {
